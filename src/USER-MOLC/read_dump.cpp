...
enum{ID,TYPE,X,Y,Z,VX,VY,VZ,Q,IX,IY,IZ,FX,FY,FZ,QW,QX,QY,QZ,ANGMOMX,ANGMOMY,ANGMOMZ,MOLID};
  ...
  for (i = 0; i < nnew; i++) {
    ...
    if (replaceflag) {
      ...
      for (ifield = 1; ifield < nfield; ifield++) {
        ...
        case QW:
	  if (bonus)
	    bonus[ellipsoid[m]].quat[0] = fields[i][ifield];
	  else
	    error->all(FLERR,"no ellipsoids but QW requested");
          break;
        case QX:
	  if (bonus)
	    bonus[ellipsoid[m]].quat[1] = fields[i][ifield];
	  else
	    error->all(FLERR,"no ellipsoids but QX requested");
          break;
        case QY:
	  if (bonus)
          bonus[ellipsoid[m]].quat[2] = fields[i][ifield];
	  else
	    error->all(FLERR,"no ellipsoids but QY requested");
          break;
        case QZ:
	  if (bonus)
          bonus[ellipsoid[m]].quat[3] = fields[i][ifield];
	  else
	    error->all(FLERR,"no ellipsoids but QZ requested");
          break;
        case ANGMOMX:
          angmom[m][0] = fields[i][ifield];
          break;
        case ANGMOMY:
          angmom[m][1] = fields[i][ifield];
          break;
        case ANGMOMZ:
          angmom[m][2] = fields[i][ifield];
          break;
        case MOLID:
          molecule[m] = static_cast<int> (fields[i][ifield]);
          break;
        }
      }
...
int ReadDump::whichtype(char *str)
{
  ...
  else if (strncmp(str,"x", 1) == 0) type = X;
  else if (strncmp(str,"y", 1) == 0) type = Y;
  else if (strncmp(str,"z", 1) == 0) type = Z;
  ...
  else if (strcmp(str,"qw") == 0) type = QW;
  else if (strcmp(str,"qx") == 0) type = QX;
  else if (strcmp(str,"qy") == 0) type = QY;
  else if (strcmp(str,"qz") == 0) type = QZ;
  else if (strcmp(str,"angmomx") == 0) type = ANGMOMX;
  else if (strcmp(str,"angmomy") == 0) type = ANGMOMY;
  else if (strcmp(str,"angmomz") == 0) type = ANGMOMZ;
  else if (strcmp(str,"mol") == 0) type = MOLID;
  return type;
}
