...
/* ----------------------------------------------------------------------
   ...
   Matteo Ricci, adapted to offcentre charges
   ------------------------------------------------------------------------- */

#include "fix_adapt_fep_offcentre_charges.h"
#include <cstring>
#include "atom.h"
#include "update.h"
#include "group.h"
...
enum{DIAMETER,CHARGE, CHARGE_OFFCENTRE};

/* ---------------------------------------------------------------------- */

FixAdaptFEPOffcentreCharges::FixAdaptFEPOffcentreCharges(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"pair") == 0) {
      ...
      if (strcmp(arg[iarg+1],"diameter") == 0) {
        ...
        chgflag = 1;
      } else if (strcmp(arg[iarg+1],"charge/offcentre") == 0) {
        adapt[nadapt].aparam = CHARGE_OFFCENTRE;
        chgoffflag = 1;
      } else error->all(FLERR,"Illegal fix adapt/fep command");
...
FixAdaptFEPOffcentreCharges::~FixAdaptFEPOffcentreCharges()
{
  for (int m = 0; m < nadapt; m++) {
    delete [] adapt[m].var;
    if (adapt[m].which == PAIR) {
...
int FixAdaptFEPOffcentreCharges::setmask()
{
  int mask = 0;
  mask |= PRE_FORCE;
  mask |= POST_RUN;
...
void FixAdaptFEPOffcentreCharges::post_constructor()
{
  if (!resetflag) return;
  if (!diamflag && !chgflag && !chgoffflag) return;
  ...
  if (chgoffflag) {
    int n = strlen(id) + strlen("_FIX_STORE_CHGOFF") + 1;
    id_fix_chgoff = new char[n];
    strcpy(id_fix_chgoff,id);
    strcat(id_fix_chgoff,"_FIX_STORE_CHGOFF");
    newarg[0] = id_fix_chgoff;
    modify->add_fix(6,newarg);
    fix_chgoff = (FixStore *) modify->fix[modify->nfix-1];

    if (fix_chg->restart_reset) fix_chgoff->restart_reset = 0;
    else {
      double *vec = fix_chgoff->vstore;
      ...
      for (int i = 0; i < nlocal; i++) {
        int atype = type[i];
        int nsites = ((PPPMOffcentre*)force->kspace)->getNsitesOf(atype);
        double* qoff = ((PPPMOffcentre*)force->kspace)->getCharges(atype);
        for (int s = 1; s <= nsites; ++s) {
          double qs = qoff[s];
          if (mask[i] & groupbit) vec[i+s-1] = qs;
          else vec[i] = 0.0;
        }
      }
    }
...
void FixAdaptFEPOffcentreCharges::init()
{
  int i,j;
...
void FixAdaptFEPOffcentreCharges::setup_pre_force(int /*vflag*/)
{
  change_settings();
}
...
void FixAdaptFEPOffcentreCharges::setup_pre_force_respa(int vflag, int ilevel)
{
...
void FixAdaptFEPOffcentreCharges::pre_force(int /*vflag*/)
{
  if (nevery == 0) return;
...
void FixAdaptFEPOffcentreCharges::pre_force_respa(int vflag, int ilevel, int)
{
  if (ilevel < nlevels_respa-1) return;
  pre_force(vflag);
}
...
void FixAdaptFEPOffcentreCharges::post_run()
{
...
void FixAdaptFEPOffcentreCharges::change_settings()
{
  int i,j;
  ...
  for (int m = 0; m < nadapt; m++) {
    ...
    if (ad->which == PAIR) {
      ...
      *kspace_scale = value;
      ...
      } else if (ad->aparam == CHARGE_OFFCENTRE) {
        int *atype = atom->type;
        int *mask = atom->mask;
        int nlocal = atom->nlocal;
        int nall = nlocal + atom->nghost;
...
void FixAdaptFEPOffcentreCharges::restore_settings()
{
  for (int m = 0; m < nadapt; m++) {
    Adapt *ad = &adapt[m];
    if (ad->which == PAIR) {
...
void FixAdaptFEPOffcentreCharges::set_arrays(int i)
{
