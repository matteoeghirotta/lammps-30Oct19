...
FixStyle(adapt/fep/offcentre,FixAdaptFEPOffcentreCharges)
...
#include "fix.h"
  ...
  namespace LAMMPS_NS {
    ...
    class FixAdaptFEPOffcentreCharges : public Fix {
    public:
      int diamflag;        // 1 if atom diameters will vary, for AtomVecGranular
      int chgflag;
      int chgoffflag;

      FixAdaptFEPOffcentreCharges(class LAMMPS *, int, char **);
      ~FixAdaptFEPOffcentreCharges();
      int setmask();
    ...
    private:
      ...
      int anypair;
      int nlevels_respa;
      char *id_fix_diam,*id_fix_chg, *id_fix_chgoff;
      class FixStore *fix_diam,*fix_chg,*fix_chgoff;
