                                 ...
                                 int &xflag, int &yflag, int &zflag)
{
  ...
  for (int i = 0; i < nfield; i++) {
   ...
   else if (fieldtype[i] == QW)
      fieldindex[i] = find_label("qw",nwords,labels);
   else if (fieldtype[i] == QX)
      fieldindex[i] = find_label("qx",nwords,labels);
    else if (fieldtype[i] == QY)
      fieldindex[i] = find_label("qy",nwords,labels);
    else if (fieldtype[i] == QZ)
      fieldindex[i] = find_label("qz",nwords,labels);

    else if (fieldtype[i] == ANGMOMX)
      fieldindex[i] = find_label("angmomx",nwords,labels);
    else if (fieldtype[i] == ANGMOMY)
      fieldindex[i] = find_label("angmomy",nwords,labels);
    else if (fieldtype[i] == ANGMOMZ)
      fieldindex[i] = find_label("angmomz",nwords,labels);

    else if (fieldtype[i] == MOLID)
      fieldindex[i] = find_label("mol",nwords,labels);
