...
#include "compute_fep_offcentre_charges.h"

using namespace LAMMPS_NS;

enum{PAIR,ATOM};
enum{CHARGE, CHARGE_OFFCENTRE};

/* ---------------------------------------------------------------------- */

ComputeFEPOffcentreCharges::ComputeFEPOffcentreCharges(LAMMPS *lmp, int narg, char **arg) :
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"pair") == 0) {
      ...
      if (strcmp(arg[iarg+1],"charge") == 0) {
        ...
        perturb[npert].aparam = CHARGE_OFFCENTRE;
        chgoffflag = 1;
...
ComputeFEPOffcentreCharges::~ComputeFEPOffcentreCharges()
{
  delete [] vector;
...
void ComputeFEPOffcentreCharges::init()
{
  int i,j;
...
void ComputeFEPOffcentreCharges::compute_vector()
{
  double pe0,pe1;
  ...
  if (chgflag || chgoffflag && force->kspace && force->kspace->compute_flag) {
...
double ComputeFEPOffcentreCharges::compute_epair()
{
  double eng, eng_pair;
  ...
  if (chgflag || chgoffflag && force->kspace) eng_pair += force->kspace->energy;
...
void ComputeFEPOffcentreCharges::perturb_params()
{
  int i,j;
  ...
  if (chgflag || chgoffflag && force->kspace) force->kspace->qsum_qsq();
}
...
void ComputeFEPOffcentreCharges::backup_params()
{
  int i,j;
...
void ComputeFEPOffcentreCharges::restore_params()
{
  int i,j;
...
void ComputeFEPOffcentreCharges::allocate_storage()
{
  nmax = atom->nmax;
...
void ComputeFEPOffcentreCharges::deallocate_storage()
{
...
void ComputeFEPOffcentreCharges::backup_qfev()
{
  int i;
...
void ComputeFEPOffcentreCharges::restore_qfev()
{
  int i;
