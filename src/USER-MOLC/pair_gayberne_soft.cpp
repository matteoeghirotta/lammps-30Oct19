...
PairGayBerneSoft::PairGayBerneSoft(LAMMPS *lmp) : Pair(lmp)
{
  if (lmp->citeme) lmp->citeme->add(cite_pair_gayberne_soft);
...
void PairGayBerneSoft::compute(int eflag, int vflag)
{
...
void PairGayBerneSoft::settings(int narg, char **arg)
{
  ...
  nlambda = force->numeric(FLERR,arg[3]);
...
double PairGayBerneSoft::init_one(int i, int j)
{
  ...
  // ON FROM HERE
  double soft_factor = pow(lambda[i][j], nlambda);
  double alpha_factor = alphalj * (1.0 - lambda[i][j])*(1.0 - lambda[i][j]);
  lj1[i][j] = soft_factor * 48.0 * epsilon[i][j] * pow(sigma[i][j],12.0);
  lj2[i][j] = soft_factor * 24.0 * epsilon[i][j] * pow(sigma[i][j],6.0);
  lj3[i][j] = soft_factor * 4.0 * epsilon[i][j] * pow(sigma[i][j],12.0);
  lj4[i][j] = soft_factor * 4.0 * epsilon[i][j] * pow(sigma[i][j],6.0);
  lj5[i][j] = alpha_factor;
                                           ...
                                           double *ttor, double *rtor)
{
  ...
  double soft_factor = pow(lambda[type[i]][type[j]], nlambda);
  double alpha_factor = alphalj * (1.0 - lambda[type[i]][type[j]])*(1.0 - lambda[type[i]][type[j]]);
  double alpha_factor_2 = alpha_factor * alpha_factor;

  double varrho = sigma[type[i]][type[j]]/(h12+gamma*sigma[type[i]][type[j]]);
  double varrho5 = pow(varrho,5.0);
  double varrho6 = pow(varrho,6.0);
  double varrho7 = varrho6*varrho;
  double varrho11 = varrho6*varrho6/varrho;
                                     ...
                                     double *ttor)
{
  ...
  double varrho6 = pow(varrho,6.0);
  double varrho12 = varrho6*varrho6;
  double u_r = 4.0* soft_factor *epsilon[type[i]][type[j]]*(varrho12-varrho6);
  ...
  temp1 = soft_factor*temp1*epsilon[type[i]][type[j]];
  double u_slj = temp1*pow(sigma12,3.0)/2.0;
  double dUr[3];
  temp2 = MathExtra::dot3(kappa,r12hat);
  double uslj_rsq = u_slj/rsq;
...
void *PairGayBerneSoft::extract(const char *str, int &dim)
{
  dim = 2;
